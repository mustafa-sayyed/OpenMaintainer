...
export const createPullRequestTools = async (pullRequest: PullRequestEvent) => {
    ...
    getPullRequest: tool({
      ...
      execute: async () => {
        try {
          ...
          console.error('Error while executing getPullRequestTool', error);
          return {
      ...
      execute: async () => {
        try {
          ...
          if (updateType !== 'minor' && updateType !== 'patch') {
            return {
              error: 'Only minor and patch Dependabot updates can be merged.',
          ...
          if (currentPullRequest.mergeable_state !== 'clean') {
            return {
              error: 'Pull request is not ready to merge cleanly.',
          ...
          console.error('Error while executing mergePullRequestTool', error);
          return {
